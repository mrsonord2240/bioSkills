...
**lavaan >= 0.7.0 breaks 3 of the 4 core functions -- this is not a floor, it is a ceiling.** `usermodel()`, `commonfactorGWAS()`, and `userGWAS()` crash with `object 'ReorderModel'`/`'ReorderModelnoSNP' not found` under BOTH `estimation='DWLS'` and `'ML'`, confirmed on GenomicSEM 0.0.5 + lavaan 0.7.2. Root cause: lavaan >=0.7.0 requires an explicit `ordered = FALSE` argument to run DWLS on continuous (non-categorical) data; GenomicSEM 0.0.5's internal `sem()` calls in these three functions' reorder step never supply it, and that step is not gated by the caller's `estimation` argument at all, so no user-facing workaround exists other than pinning lavaan. `commonfactor()` alone has a working escape hatch: its DWLS attempt fails the same way, but it internally retries with ML -- except it reports the failure as **"The common factor model failed to converge on a solution. Please try specifying an alternative model"**, which misdiagnoses the cause: this is a lavaan-version argument-compatibility crash, not a non-convergent fit, and re-specifying the model will not fix it. If that message appears together with an underlying `object '...Results' not found` error visible via `traceback()`, pin lavaan instead of re-specifying. Checked against GenomicSEM's GitHub HEAD (commit `6b65ca5`, 2026-08-26): the unconditional `estimator="DWLS"` reorder-step calls inside `usermodel()`/`commonfactorGWAS()`/`userGWAS()` are unchanged upstream as of that date -- upgrading GenomicSEM does not fix this. **Confirmed working**: GenomicSEM 0.0.5 + lavaan 0.6.19 runs `commonfactor()`, `usermodel()`, `commonfactorGWAS()`, and `userGWAS()` correctly under both DWLS and ML, recovering exact planted loadings/factor-correlations on synthetic covariance/GWAS inputs (see Tool Installation).
